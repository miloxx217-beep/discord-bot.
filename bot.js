...
Kliknij odpowiedni przycisk, aby otworzyć ticket.`);
...
Wpisz dane w jednej wiadomości.`);
...
Opisz dokładnie swoje pytanie lub problem.`);
    ...
    if (interaction.customId === "ticket_accept") {
        ...
        if (interaction.user.id !== interaction.guild.ownerId)
            return interaction.reply({ content: "Tylko właściciel może przyjąć sprawę.", ephemeral: true });

        return interaction.reply({ content: "Sprawa została przyjęta przez właściciela." });
    }
